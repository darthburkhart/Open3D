...
#include "RegistrationCuda2.h"

namespace open3d {
namespace cuda {

__global__
void ComputeColorGradientKernel2(
    RegistrationCudaDevice2 estimation,
...
void RegistrationCudaKernelCaller2::ComputeColorGradeint(
    RegistrationCuda2 &estimation,
    ...
    ComputeColorGradientKernel2 << < blocks, threads >> > (
...
__global__
void BuildLinearSystemForColoredICPKernel2(
    RegistrationCudaDevice2 estimation) {
...
void RegistrationCudaKernelCaller2::BuildLinearSystemForColoredICP(
    ...
    BuildLinearSystemForColoredICPKernel2 << < blocks, threads >>
        > (
...
__global__
void BuildLinearSystemForPointToPlaneICPKernel2(
    RegistrationCudaDevice2 estimation) {
    __shared__ float local_sum0[THREAD_1D_UNIT];
    __shared__ float local_sum1[THREAD_1D_UNIT];
    __shared__ float local_sum2[THREAD_1D_UNIT];
...
void RegistrationCudaKernelCaller2::BuildLinearSystemForPointToPlaneICP(
    RegistrationCuda2 &registration) {
    const dim3 blocks(DIV_CEILING(registration.correspondences_.indices_.size(),
                                  THREAD_1D_UNIT));
    const dim3 threads(THREAD_1D_UNIT);

    BuildLinearSystemForPointToPlaneICPKernel2 << < blocks,
        threads >> > (
            *registration.device_);
    CheckCuda(cudaDeviceSynchronize());
    CheckCuda(cudaGetLastError());
...
__global__
void ComputeSumForPointToPointICPKernel2(
    RegistrationCudaDevice2 estimation) {
    __shared__ float local_sum0[THREAD_1D_UNIT];
    __shared__ float local_sum1[THREAD_1D_UNIT];
    __shared__ float local_sum2[THREAD_1D_UNIT];
...
void RegistrationCudaKernelCaller2::ComputeSumForPointToPointICP(
    RegistrationCuda2 &registration) {
    const dim3 blocks(DIV_CEILING(registration.correspondences_.indices_.size(),
                                  THREAD_1D_UNIT));
    const dim3 threads(THREAD_1D_UNIT);

    ComputeSumForPointToPointICPKernel2 << < blocks, threads >> > (*registration.device_);
    CheckCuda(cudaDeviceSynchronize());
    CheckCuda(cudaGetLastError());
}

__global__
void BuildLinearSystemForPointToPointICPKernel2(
    RegistrationCudaDevice2 estimation,
...
void RegistrationCudaKernelCaller2::BuildLinearSystemForPointToPointICP(
    RegistrationCuda2 &registration,
    ...
    BuildLinearSystemForPointToPointICPKernel2<< < blocks, threads >> > (
...
__global__
void ComputeInformationMatrixKernel2(RegistrationCudaDevice2 estimation) {
    __shared__ float local_sum0[THREAD_1D_UNIT];
    __shared__ float local_sum1[THREAD_1D_UNIT];
    __shared__ float local_sum2[THREAD_1D_UNIT];
...
void RegistrationCudaKernelCaller2::ComputeInformationMatrix(
    RegistrationCuda2 &estimation) {
    const dim3 blocks(DIV_CEILING(estimation.correspondences_.indices_.size(),
                                  THREAD_1D_UNIT));
    const dim3 threads(THREAD_1D_UNIT);

    ComputeInformationMatrixKernel2 << < blocks, threads >> > (
        *estimation.device_);
    CheckCuda(cudaDeviceSynchronize());
    CheckCuda(cudaGetLastError());
}
