...
/* Preparation */
RegistrationCuda2::RegistrationCuda2(
...
void RegistrationCuda2::Create(
    const registration::TransformationEstimationType &type) {
    type_ = type;
    lambda_geometric_ = 0.968;

    device_ = std::make_shared<RegistrationCudaDevice2>();
    results_.Create(28);
}

void RegistrationCuda2::Release() {
...
void RegistrationCuda2::UpdateDevice() {
    if (device_ != nullptr) {
...
void RegistrationCuda2::Initialize(
...
/* High-level API */
RegistrationResultCuda2 RegistrationCuda2::ComputeICP(int iter) {
    assert(iter > 0);
...
Eigen::Matrix6d RegistrationCuda2::ComputeInformationMatrix() {
    ...
    RegistrationResultCuda2 result;

    results_.Memset(0);
    RegistrationCudaKernelCaller2::ComputeInformationMatrix(*this);
...
/* ICP */
RegistrationResultCuda2 RegistrationCuda2::DoSingleIteration(int iter) {
    RegistrationResultCuda2 delta;
...
void RegistrationCuda2::GetCorrespondences() {
#ifdef _OPENMP
#pragma omp parallel
    {
#endif
...
void RegistrationCuda2::TransformSourcePointCloud(
    const Eigen::Matrix4d &source_to_target) {
    source_.Transform(source_to_target);
    source_cpu_.Transform(source_to_target);
}

RegistrationResultCuda2 RegistrationCuda2::BuildAndSolveLinearSystem() {
    RegistrationResultCuda2 result;
...
void RegistrationCuda2::ExtractResults(
...
void RegistrationCuda2::ComputeColorGradients(
    geometry::PointCloud &target,
...
#pragma omp for nowait
    ...
    RegistrationCudaKernelCaller2::ComputeColorGradeint(
        *this, corres_for_color_gradient);
}

RegistrationResultCuda2 RegistrationCuda2::Umeyama() {
    RegistrationResultCuda2 result;
    ...
    results_.Memset(0);
    RegistrationCudaKernelCaller2::ComputeSumForPointToPointICP(*this);
    std::vector<float> downloaded_result = results_.DownloadAll();
    ...
    results_.Memset(0);
    RegistrationCudaKernelCaller2::BuildLinearSystemForPointToPointICP(
